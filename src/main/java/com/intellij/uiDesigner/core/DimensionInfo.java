...
public abstract class DimensionInfo {
	...
	public DimensionInfo(LayoutState layoutState, int gap) {
		if(layoutState == null) {
			...
			List<Integer> elimit = new ArrayList<Integer>();
			this.mySpansAfterElimination = this.mySpan.clone();
			Util.eliminate(this.myCell.clone(), this.mySpansAfterElimination, elimit);
			this.myCellSizePolicies = new int[this.getCellCount()];
			for(int i = 0; i < this.myCellSizePolicies.length; ++i) {
				this.myCellSizePolicies[i] = this.getCellSizePolicyImpl(i, elimit);
			}
		}
	}
	...
	public abstract DimensionInfo getDimensionInfo(GridLayoutManager manager);
	...
	protected abstract int getSizePolicy(int componentIndex);

	protected abstract int getChildLayoutCellCount(GridLayoutManager manager);
	...
	private int getCellSizePolicyImpl(int cellIndex, List<Integer> eliminatedCells) {
